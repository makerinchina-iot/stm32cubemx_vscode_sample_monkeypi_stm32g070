...
void st7789_fill_area(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *buff)
{
    st7789_set_pos(x, y, x + w - 1, y + h - 1);

    tft_port_write_multi_dat16((uint8_t *)buff, w * h);

    // for (uint32_t i = 0; i < w * h; i++)
    // {
    //     tft_port_write_dat16(*buff);
    //     buff++;
    // }
}